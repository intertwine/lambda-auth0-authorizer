...
_Promise.promisifyAll(Object.getPrototypeOf(dynamo));
...
var getPrincipalId = function(userInfo) {
    if (!userInfo || !userInfo[uidkey]) {
        throw new Error('No user_id returned from Auth0');
    }
    console.log('Auth0 authentication successful for user_id ' + userInfo[uidkey]);

    return userInfo[uidkey];
};
